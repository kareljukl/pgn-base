...
- **ŠSČR competition hooks** (`hooks/useChessczCompetition.ts`) — `useChessczDetails`, `useChessczTable`, `useChessczCompSchedule` (full-season rounds + matches), `useChessczTeamSchedule`, `useChessczRoster` (team lineup for roster-only autocomplete), `useChessczRoundSchedule`, `useChessczRoundMatches` over the cached proxy endpoints; `fetchChessczRoundMatches(compId, round)` for imperative use in DatabaseDetail's "Načíst výsledky" handler. All hooks accept `null` ids and gate with `enabled`.
